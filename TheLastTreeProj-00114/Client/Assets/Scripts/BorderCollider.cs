...
public class BorderCollider : MonoBehaviour {
    ...
    void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.transform.gameObject.name.Contains("fantan") || collision.transform.gameObject.name.Contains("JingGuBang"))
        {
            ...
            collision.gameObject.transform.GetComponent<Rigidbody2D>().AddForce(new Vector2(direct.x * GameData.Instance.protectPower/2, direct.y * GameData.Instance.protectPower/2));
